...
MtrFindCmtDfr <- function(mt.list)
{
  ...
  for( n in seq_along(rowNum) )
  {
    ...
    rDfr        <- data.frame("cmt", openNum, indentNum[1] )
    names(rDfr) <- names(retDfr)
    retDfr      <- rbind(retDfr, rDfr)
  }
  retDfr
}
MtrFindLoopDfr <- function(mt.list)
{
  endNum <- length(mt.list)
  retDfr <- dataFrame( colClasses=c(Token="character", Open="numeric", 
  ...
  for( n in seq_along(rowNum) )
  {
    ...
    rDfr        <- data.frame("for", openNum, mRow-1, indentNum[1] )
    names(rDfr) <- names(retDfr)
    retDfr      <- rbind(retDfr, rDfr)
  }
  retDfr
}
MtrFindCmtDfr(mt.list)
MtrFindLoopDfr(mt.list)
sql.list <- list()
j <- 1
