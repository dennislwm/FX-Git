...
int start() {
   ...
   for (int pos_456 = OrdersTotal() - 1; pos_456 >= 0; pos_456--) {
      if (!OrderSelect(pos_456, SELECT_BY_POS, MODE_TRADES)) Print("Error in OrderSelect! Position:", pos_456);
      else {
         if (OrderType() <= OP_SELL && OrderSymbol() == Symbol()) {
            if (OrderMagicNumber() == Magic1) {
               if (OrderType() == OP_BUY) {
                  if (OrderStopLoss() == 0.0 && Hidden_StopAndTarget == FALSE) {
                     price_460 = NormalizeDouble(OrderOpenPrice() - li_276 * ld_88, Digits);
                     price_468 = NormalizeDouble(OrderOpenPrice() + li_272 * ld_88, Digits);
                     OrderModify(OrderTicket(), OrderOpenPrice(), price_460, price_468, 0, Green);
                  }
                  if ((f0_20(gi_828, iwpr_168, OSC_close) == 0 && Bid > iclose_152 + gi_472 * ld_88) || Bid >= OrderOpenPrice() + li_272 * ld_88 || Bid <= OrderOpenPrice() - li_276 * ld_88) {
                     RefreshRates();
                  ...
                  if ((f0_20(gi_828, iwpr_168, OSC_close) == 1 && Bid < iclose_152 - gi_472 * ld_88) || Ask <= OrderOpenPrice() - li_272 * ld_88 || Ask >= OrderOpenPrice() + li_276 * ld_88) {
                     RefreshRates();
            ...
            if (OrderMagicNumber() == Magic2) {
               if (OrderType() == OP_BUY) {
                  if (OrderStopLoss() == 0.0 && Hidden_StopAndTarget == FALSE) {
                     price_476 = NormalizeDouble(OrderOpenPrice() - li_284 * ld_88, Digits);
                     price_484 = NormalizeDouble(OrderOpenPrice() + li_280 * ld_88, Digits);
                     OrderModify(OrderTicket(), OrderOpenPrice(), price_476, price_484, 0, Green);
                  }
                  if ((f0_22(gi_828, iclose_208, ld_200, ld_192, Break * ld_88) == 0 && TimeCurrent() - OrderOpenTime() > 3600) || Bid >= OrderOpenPrice() + li_280 * ld_88 || Bid <= OrderOpenPrice() - li_284 * ld_88) {
                     RefreshRates();
                     OrderClose(OrderTicket(), OrderLots(), NormalizeDouble(Bid, Digits), slippage_380, Violet);
                     Sleep(5000);
                  } else count_392++;
                  if (TimeCurrent() >= li_420) {
                     if (Use_Exp_Trailing_II) {
                        ld_492 = f0_8(Exp_Trail_Factor_II * ld_88, OrderOpenPrice(), iHigh(NULL, PERIOD_M5, iHighest(NULL, PERIOD_M5, MODE_HIGH, iBarShift(NULL, PERIOD_M5, OrderOpenTime()) +
                           1, 0)));
                     ...
                     if (Bid - OrderOpenPrice() > F_TrailingProfit_II * ld_88 && (!Use_Exp_Trailing_II)) ld_492 = F_Trailing_II * ld_88;
                     price_500 = NormalizeDouble(Bid - ld_492, Digits);
                     if (Hidden_StopAndTarget) {
                        if (TimeCurrent() - OrderOpenTime() > 60 && Bid <= MathMax(OrderOpenPrice() - li_284 * ld_88, iHigh(NULL, PERIOD_M5, iHighest(NULL, PERIOD_M5, MODE_HIGH, iBarShift(NULL,
                           PERIOD_M5, OrderOpenTime()) + 1, 0)) - ld_492) && iHigh(NULL, PERIOD_M5, iHighest(NULL, PERIOD_M5, MODE_HIGH, iBarShift(NULL, PERIOD_M5, OrderOpenTime()) + 1, 0)) - OrderOpenPrice() > ld_492) {
                           RefreshRates();
                           OrderClose(OrderTicket(), OrderLots(), NormalizeDouble(Bid, Digits), slippage_380, Violet);
                           Sleep(5000);
                        }
                     } else {
                        if (Bid - OrderOpenPrice() > ld_492) {
                           if (OrderStopLoss() <= price_500 - Point) {
                  ...
                  if ((f0_22(gi_828, iclose_208, ld_200, ld_192, Break * ld_88) == 1 && TimeCurrent() - OrderOpenTime() > 3600) || Ask <= OrderOpenPrice() - li_280 * ld_88 || Ask >= OrderOpenPrice() +
                     li_284 * ld_88) {
                     RefreshRates();
                     OrderClose(OrderTicket(), OrderLots(), NormalizeDouble(Ask, Digits), slippage_380, Violet);
                     Sleep(5000);
                  ...
                  if (TimeCurrent() >= li_420) {
                     if (Use_Exp_Trailing_II) {
                        ld_492 = f0_14(Exp_Trail_Factor_II * ld_88, OrderOpenPrice(), iLow(NULL, PERIOD_M5, iLowest(NULL, PERIOD_M5, MODE_LOW, iBarShift(NULL, PERIOD_M5, OrderOpenTime()) +
                           1, 0)) + Ask - Bid);
                     } else ld_492 = iatr_176 * ATRTrailingFactor2;
                     if (ld_492 > MaxPipsTrailing2 * ld_88) ld_492 = MaxPipsTrailing2 * ld_88;
                     if (ld_492 < MinPipsTrailing2 * ld_88) ld_492 = MinPipsTrailing2 * ld_88;
                     if (OrderOpenPrice() - Ask > F_TrailingProfit_II * ld_88 && (!Use_Exp_Trailing_II)) ld_492 = F_Trailing_II * ld_88;
                     price_500 = NormalizeDouble(Ask + ld_492, Digits);
                     if (Hidden_StopAndTarget) {
                        if (TimeCurrent() - OrderOpenTime() > 60 && Ask >= Ask - Bid + MathMin(OrderOpenPrice() + li_284 * ld_88, iLow(NULL, PERIOD_M5, iLowest(NULL, PERIOD_M5, MODE_LOW,
                           iBarShift(NULL, PERIOD_M5, OrderOpenTime()) + 1, 0)) + ld_492) && OrderOpenPrice() - (iLow(NULL, PERIOD_M5, iLowest(NULL, PERIOD_M5, MODE_LOW, iBarShift(NULL, PERIOD_M5,
                           OrderOpenTime()) + 1, 0)) + Ask - Bid) > ld_492) {
                           RefreshRates();
                           OrderClose(OrderTicket(), OrderLots(), NormalizeDouble(Ask, Digits), slippage_380, Violet);
                           Sleep(5000);
                        }
                     } else {
                        if (OrderOpenPrice() - Ask > ld_492) {
                           if (OrderStopLoss() >= price_500 + Point) {
            ...
            if (OrderMagicNumber() == Magic3) {
               if (OrderType() == OP_BUY) {
                  if (OrderStopLoss() == 0.0 && Hidden_StopAndTarget == FALSE) {
                     price_508 = NormalizeDouble(OrderOpenPrice() - li_292 * ld_88, Digits);
                     price_516 = NormalizeDouble(OrderOpenPrice() + li_288 * ld_88, Digits);
                  ...
                  if (((li_308 <= li_312 && TimeHour(TimeCurrent()) >= li_308 && TimeHour(TimeCurrent()) <= li_312) || (li_308 > li_312 && TimeHour(TimeCurrent()) >= li_308 || TimeHour(TimeCurrent()) <= li_312) &&
                     f0_18(gi_828, ibands_240, ibands_248, gi_700 * ld_88, ihigh_224, ilow_232, gi_696 * ld_88) == 0 && TimeCurrent() - OrderOpenTime() > 7200) || Bid >= OrderOpenPrice() +
                     li_288 * ld_88 || Bid <= OrderOpenPrice() - li_292 * ld_88) {
                     RefreshRates();
                     OrderClose(OrderTicket(), OrderLots(), NormalizeDouble(Bid, Digits), slippage_380, Violet);
                     Sleep(5000);
                  } else count_400++;
                  if (TimeCurrent() >= li_428) {
                     if (Use_Exp_Trailing_III) {
                        ld_524 = Exp_Trail_Factor_III * ld_88 / MathMax(ld_88, iHigh(NULL, PERIOD_M5, iHighest(NULL, PERIOD_M5, MODE_HIGH, iBarShift(NULL, PERIOD_M5, OrderOpenTime()) + 1,
                           0)) - OrderOpenPrice());
                     ...
                     if (Bid - OrderOpenPrice() > F_TrailingProfit_III * ld_88 && (!Use_Exp_Trailing_III)) ld_524 = F_Trailing_III * ld_88;
                     price_532 = NormalizeDouble(Bid - ld_524, Digits);
                     if (Hidden_StopAndTarget) {
                        if (TimeCurrent() - OrderOpenTime() > 300 && Bid <= MathMax(OrderOpenPrice() - li_292 * ld_88, iHigh(NULL, PERIOD_M5, iHighest(NULL, PERIOD_M5, MODE_HIGH, iBarShift(NULL,
                           PERIOD_M5, OrderOpenTime()) + 1, 0)) - ld_524) && iHigh(NULL, PERIOD_M5, iHighest(NULL, PERIOD_M5, MODE_HIGH, iBarShift(NULL, PERIOD_M5, OrderOpenTime()) + 1, 0)) - OrderOpenPrice() > ld_524) {
                           RefreshRates();
                           OrderClose(OrderTicket(), OrderLots(), NormalizeDouble(Bid, Digits), slippage_380, Violet);
                           Sleep(5000);
                        }
                     } else {
                        if (Bid - OrderOpenPrice() > ld_524) {
                           if (OrderStopLoss() <= price_532 - Point) {
                  ...
                  if (((li_308 <= li_312 && TimeHour(TimeCurrent()) >= li_308 && TimeHour(TimeCurrent()) <= li_312) || (li_308 > li_312 && TimeHour(TimeCurrent()) >= li_308 || TimeHour(TimeCurrent()) <= li_312) &&
                     f0_18(gi_828, ibands_240, ibands_248, gi_700 * ld_88, ihigh_224, ilow_232, gi_696 * ld_88) == 1 && TimeCurrent() - OrderOpenTime() > 7200) || Ask <= OrderOpenPrice() - li_288 * ld_88 ||
                     Ask >= OrderOpenPrice() + li_292 * ld_88) {
                     RefreshRates();
                     OrderClose(OrderTicket(), OrderLots(), NormalizeDouble(Ask, Digits), slippage_380, Violet);
                     Sleep(5000);
                  } else count_404++;
                  if (TimeCurrent() >= li_428) {
                     if (Use_Exp_Trailing_III) {
                        ld_524 = Exp_Trail_Factor_III * ld_88 / MathMax(ld_88, OrderOpenPrice() - (iLow(NULL, PERIOD_M5, iLowest(NULL, PERIOD_M5, MODE_LOW, iBarShift(NULL, PERIOD_M5, OrderOpenTime()) +
                           1, 0)) + Ask - Bid));
                     } else ld_524 = iatr_216 * gd_684;
                     if (ld_524 > MaxPipsTrailing3 * ld_88) ld_524 = MaxPipsTrailing3 * ld_88;
                     if (ld_524 < MinPipsTrailing3 * ld_88) ld_524 = MinPipsTrailing3 * ld_88;
                     if (OrderOpenPrice() - Ask > F_TrailingProfit_III * ld_88 && (!Use_Exp_Trailing_III)) ld_524 = F_Trailing_III * ld_88;
                     price_532 = NormalizeDouble(Ask + ld_524, Digits);
                     if (Hidden_StopAndTarget) {
                        if (TimeCurrent() - OrderOpenTime() > 300 && Ask >= Ask - Bid + MathMin(OrderOpenPrice() + li_292 * ld_88, iLow(NULL, PERIOD_M5, iLowest(NULL, PERIOD_M5, MODE_LOW,
                           iBarShift(NULL, PERIOD_M5, OrderOpenTime()) + 1, 0)) + ld_524) && OrderOpenPrice() - (iLow(NULL, PERIOD_M5, iLowest(NULL, PERIOD_M5, MODE_LOW, iBarShift(NULL, PERIOD_M5,
                           OrderOpenTime()) + 1, 0)) + Ask - Bid) > ld_524) {
                           RefreshRates();
                           OrderClose(OrderTicket(), OrderLots(), NormalizeDouble(Ask, Digits), slippage_380, Violet);
                           Sleep(5000);
                        }
                     } else {
                        if (OrderOpenPrice() - Ask > ld_524) {
                           if (OrderStopLoss() >= price_532 + Point) {
            ...
            if (OrderMagicNumber() == Magic4) {
               if (OrderType() == OP_BUY) {
                  if (OrderStopLoss() == 0.0 && Hidden_StopAndTarget == FALSE) {
                     ...
                     price_548 = NormalizeDouble(OrderOpenPrice() + (ihigh_256 - ilow_264) * TargetPercent, Digits);
                     OrderModify(OrderTicket(), OrderOpenPrice(), price_540, price_548, 0, Green);
                  }
                  if (TimeCurrent() - OrderOpenTime() > 300 && f0_13(gi_828, iClose(NULL, ReverseTF, 1), iOpen(NULL, ReverseTF, 1), Bid - OrderOpenPrice(), ExitProfit * ld_88) == 0) {
                     RefreshRates();
                     OrderClose(OrderTicket(), OrderLots(), NormalizeDouble(Bid, Digits), slippage_380, Violet);
                     Sleep(5000);
                  } else count_408++;
                  ...
                  if (Use_Exp_Trailing_IV) {
                     ld_524 = Exp_Trail_Factor_IV * ld_88 / MathMax(ld_88, iHigh(NULL, PERIOD_M5, iHighest(NULL, PERIOD_M5, MODE_HIGH, iBarShift(NULL, PERIOD_M5, OrderOpenTime()) + 1,
                        0)) - OrderOpenPrice());
                  } else ld_524 = iatr_216 * gd_772;
                  if (ld_524 > gi_764 * ld_88) ld_524 = gi_764 * ld_88;
                  if (ld_524 < gi_768 * ld_88) ld_524 = gi_768 * ld_88;
                  if (Bid - OrderOpenPrice() > F_TrailingProfit_IV * ld_88 && (!Use_Exp_Trailing_IV)) ld_524 = F_Trailing_IV * ld_88;
                  price_532 = NormalizeDouble(Bid - ld_524, Digits);
                  if (Hidden_StopAndTarget) {
                     if (!(TimeCurrent() - OrderOpenTime() > 300 && Bid <= MathMax(OrderOpenPrice() - li_296 * ld_88, iHigh(NULL, PERIOD_M5, iHighest(NULL, PERIOD_M5, MODE_HIGH, iBarShift(NULL,
                        PERIOD_M5, OrderOpenTime()) + 1, 0)) - ld_524) && iHigh(NULL, PERIOD_M5, iHighest(NULL, PERIOD_M5, MODE_HIGH, iBarShift(NULL, PERIOD_M5, OrderOpenTime()) + 1, 0)) - OrderOpenPrice() > ld_524)) continue;
                     RefreshRates();
                     OrderClose(OrderTicket(), OrderLots(), NormalizeDouble(Bid, Digits), slippage_380, Violet);
                     Sleep(5000);
                  } else {
                     if (Bid - OrderOpenPrice() <= ld_524) continue;
                     if (OrderStopLoss() > price_532 - Point) continue;
               ...
               if (TimeCurrent() - OrderOpenTime() > 300 && f0_13(gi_828, iClose(NULL, ReverseTF, 1), iOpen(NULL, ReverseTF, 1), OrderOpenPrice() - Ask, ExitProfit * ld_88) == 1) {
                  RefreshRates();
                  OrderClose(OrderTicket(), OrderLots(), NormalizeDouble(Ask, Digits), slippage_380, Violet);
                  Sleep(5000);
               } else count_412++;
               if (TimeCurrent() >= li_436) {
                  if (Use_Exp_Trailing_IV) {
                     ld_524 = Exp_Trail_Factor_IV * ld_88 / MathMax(ld_88, OrderOpenPrice() - (iLow(NULL, PERIOD_M5, iLowest(NULL, PERIOD_M5, MODE_LOW, iBarShift(NULL, PERIOD_M5, OrderOpenTime()) +
                        1, 0)) + Ask - Bid));
                  } else ld_524 = iatr_216 * gd_772;
                  if (ld_524 > gi_764 * ld_88) ld_524 = gi_764 * ld_88;
                  if (ld_524 < gi_768 * ld_88) ld_524 = gi_768 * ld_88;
                  if (OrderOpenPrice() - Ask > F_TrailingProfit_III * ld_88 && (!Use_Exp_Trailing_IV)) ld_524 = F_Trailing_IV * ld_88;
                  price_532 = NormalizeDouble(Ask + ld_524, Digits);
                  if (Hidden_StopAndTarget) {
                     if (!(TimeCurrent() - OrderOpenTime() > 300 && Ask >= Ask - Bid + MathMin(OrderOpenPrice() + li_296 * ld_88, iLow(NULL, PERIOD_M5, iLowest(NULL, PERIOD_M5, MODE_LOW,
                        iBarShift(NULL, PERIOD_M5, OrderOpenTime()) + 1, 0)) + ld_524) && OrderOpenPrice() - (iLow(NULL, PERIOD_M5, iLowest(NULL, PERIOD_M5, MODE_LOW, iBarShift(NULL, PERIOD_M5,
                        OrderOpenTime()) + 1, 0)) + Ask - Bid) > ld_524)) continue;
                     RefreshRates();
                     OrderClose(OrderTicket(), OrderLots(), NormalizeDouble(Ask, Digits), slippage_380, Violet);
                     Sleep(5000);
                     continue;
   ...
   if (TradeMMSys1 > 0.0) lots_572 = MathMax(g_minlot_836, MathMin(g_maxlot_844, NormalizeDouble(f0_16() / ld_40 / 100.0 * AccountFreeMargin() / g_lotstep_860 / (g_lotsize_856 / 100), 0) * g_lotstep_860));
   if (lots_572 > MaximalLots) lots_572 = MaximalLots;
   double lots_580 = MathMin(g_maxlot_844, MathMax(g_minlot_836, LotsSys2));
   if (TradeMMSys2 > 0.0) lots_580 = MathMax(g_minlot_836, MathMin(g_maxlot_844, NormalizeDouble(f0_0() / ld_40 / 100.0 * AccountFreeMargin() / g_lotstep_860 / (g_lotsize_856 / 100), 0) * g_lotstep_860));
   if (lots_580 > MaximalLots) lots_580 = MaximalLots;
   double lots_588 = MathMin(g_maxlot_844, MathMax(g_minlot_836, LotsSys3));
   if (TradeMMSys3 > 0.0) lots_588 = MathMax(g_minlot_836, MathMin(g_maxlot_844, NormalizeDouble(f0_11() / ld_40 / 100.0 * AccountFreeMargin() / g_lotstep_860 / (g_lotsize_856 / 100), 0) * g_lotstep_860));
   if (lots_588 > MaximalLots) lots_588 = MaximalLots;
   double lots_596 = MathMin(g_maxlot_844, MathMax(g_minlot_836, LotsSys4));
   if (TradeMMSys4 > 0.0) lots_596 = MathMax(g_minlot_836, MathMin(g_maxlot_844, NormalizeDouble(f0_19() / ld_40 / 100.0 * AccountFreeMargin() / g_lotstep_860 / (g_lotsize_856 / 100), 0) * g_lotstep_860));
   ...
   if (cmd_36 >= OP_BUY) {
      if (li_84 == FALSE) ticket_440 = OrderSend(Symbol(), cmd_36, lots_572, price_0, slippage_380, 0, 0, CommentSys1, Magic1, 0, color_32);
      else ticket_440 = OrderSend(Symbol(), cmd_36, lots_572, price_0, slippage_380, price_8, price_16, CommentSys1, Magic1, 0, color_32);
      Sleep(5000);
      if (ticket_440 > 0) {
         if (OrderSelect(ticket_440, SELECT_BY_TICKET, MODE_TRADES)) Print("Order " + ls_604 + " opened!: ", OrderOpenPrice());
   ...
   if (cmd_36 >= OP_BUY) {
      if (li_84 == FALSE) ticket_444 = OrderSend(Symbol(), cmd_36, lots_580, price_0, slippage_380, 0, 0, CommentSys2, Magic2, 0, color_32);
      else ticket_444 = OrderSend(Symbol(), cmd_36, lots_580, price_0, slippage_380, price_8, price_16, CommentSys2, Magic2, 0, color_32);
      Sleep(5000);
      if (ticket_444 > 0) {
         if (OrderSelect(ticket_444, SELECT_BY_TICKET, MODE_TRADES)) Print("Order " + ls_604 + " opened!: ", OrderOpenPrice());
      } else Print("Error opening " + ls_604 + " order!: ", GetLastError());
   }
   cmd_36 = -1;
   ...
   if (cmd_36 >= OP_BUY) {
      if (li_84 == FALSE) ticket_448 = OrderSend(Symbol(), cmd_36, lots_588, price_0, slippage_380, 0, 0, CommentSys3, Magic3, 0, color_32);
      else ticket_448 = OrderSend(Symbol(), cmd_36, lots_588, price_0, slippage_380, price_8, price_16, CommentSys3, Magic3, 0, color_32);
      Sleep(5000);
      if (ticket_448 > 0) {
         if (OrderSelect(ticket_448, SELECT_BY_TICKET, MODE_TRADES)) Print("Order " + ls_604 + " opened!: ", OrderOpenPrice());
      } else Print("Error opening " + ls_604 + " order!: ", GetLastError());
   }
   cmd_36 = -1;
   ...
   if (cmd_36 >= OP_BUY) {
      if (li_84 == FALSE) ticket_452 = OrderSend(Symbol(), cmd_36, lots_596, price_0, slippage_380, 0, 0, CommentSys4, Magic4, 0, color_32);
      else ticket_452 = OrderSend(Symbol(), cmd_36, lots_596, price_0, slippage_380, price_8, price_16, CommentSys4, Magic4, 0, color_32);
      Sleep(5000);
      if (ticket_452 > 0) {
         if (OrderSelect(ticket_452, SELECT_BY_TICKET, MODE_TRADES)) Print("Order " + ls_604 + " opened!: ", OrderOpenPrice());
      } else Print("Error opening " + ls_604 + " order!: ", GetLastError());
   }
   return (0);
}
   ...
   if (g_datetime_904 != TimeCurrent()) {
      for (int pos_20 = 0; pos_20 <= OrdersTotal() - 1; pos_20++) {
         if (OrderSelect(pos_20, SELECT_BY_POS, MODE_TRADES)) {
            if (OrderType() <= OP_SELL && OrderSymbol() == Symbol()) {
               ls_0 = "";
               ls_8 = "";
               if (OrderOpenTime() >= g_datetime_904) {
      ...
      for (pos_20 = OrdersHistoryTotal() - 1; pos_20 >= 0; pos_20--) {
         if (OrderSelect(pos_20, SELECT_BY_POS, MODE_HISTORY)) {
            if (OrderType() <= OP_SELL && OrderSymbol() == Symbol()) {
               if (OrderCloseTime() <= g_datetime_904) break;
               ls_0 = "";
               ls_8 = "";
               if (OrderMagicNumber() == Magic1) ls_0 = "FX COMBO - System 1";
               else {
