//|-----------------------------------------------------------------------------------------|
//|                                                           ForexGrowthBot_v16_SqLite.mq4 |
//|                                                            Copyright © 2011, Dennis Lee |
//| Assert History                                                                          |
//| 1.20    Added PlusTurtle.mqh and PlusGhost.mqh (SqLite).                                |
//| 1.10    Added user parameter AccountNo.                                                 |
//| 1.00    Generated by EX4 TO MQ4 decompile Service for ForexGrowthBot v1.6.              |
//|-----------------------------------------------------------------------------------------|
...
int CareCloseOrder(int a_ticket_0, int ai_4, double a_lots_8) {
   ...
   if (!IsTesting()) GhostOrderClose(a_ticket_0, a_lots_8, price_32, 25, color_20);
   else {
      for (count_24 = 0; count_24 < gi_344; count_24++) {
         WaitUntilContex();
         is_closed_40 = GhostOrderClose(a_ticket_0, a_lots_8, price_32, 25, color_20);
         error_28 = 0;
...
//--- Assert 4: Dynamically resize arrays
   ...
   ld_12 = NormalizeDouble(ld_12, 2);
   if (ld_12 > 0.0) {
      if (IsTesting()) GhostOrderSend(Symbol(), OP_BUY, MathAbs(ld_12), normPrice(Ask), 25, 0, 0, g_comment_124, Magic, 0, Green);
      else {
         WaitUntilContex();
         for (count_28 = 0; count_28 < gi_344; count_28++) {
            ticket_36 = GhostOrderSend(Symbol(), OP_BUY, MathAbs(ld_12), normPrice(Ask), 25, 0, 0, g_comment_124, Magic, 0, Green);
   ...
   if (ld_12 < 0.0) {
      if (IsTesting()) GhostOrderSend(Symbol(), OP_SELL, MathAbs(ld_12), normPrice(Bid), 25, 0, 0, g_comment_124, Magic, 0, Red);
      else {
         WaitUntilContex();
         for (count_28 = 0; count_28 < gi_344; count_28++) {
            ticket_36 = GhostOrderSend(Symbol(), OP_SELL, MathAbs(ld_12), normPrice(Bid), 25, 0, 0, g_comment_124, Magic, 0, Red);
            if (ticket_36 >= 0) break;
            Sleep(gi_348);
            RefreshRates();
         }
         ...
         for (pos_28 = 0; pos_28 < gi_232; pos_28++) {
            ...
            if ((!Assign_PT_and_ST) || ManualTradeControl) {
               ...
               if (li_92 == 1) {
                  ...
                  if (IsTesting()) {
                     ticket_44 = GhostOrderSend(Symbol(), OP_BUY, LotSize, price_164, 25, 0, 0, g_comment_124, Magic, 0, Green);
                     GhostOrderModify(ticket_44, price_164, price_164 - NormalizeDouble(ld_156 * gd_156, Digits), price_164 + NormalizeDouble(ld_156 * gd_148, Digits), 0, Green);
                  } else {
                     WaitUntilContex();
                     for (count_8 = 0; count_8 < gi_344; count_8++) {
                        ticket_44 = GhostOrderSend(Symbol(), OP_BUY, LotSize, price_164, 25, 0, 0, g_comment_124, Magic, 0, Green);
                     ...
                     if (ticket_44 >= 0 && (!ManualTradeControl)) {
                        for (count_8 = 0; count_8 < gi_344; count_8++) {
                           if (!(!GhostOrderModify(ticket_44, price_164, normPrice(price_164) - NormalizeDouble(ld_156 * gd_156, Digits), normPrice(price_164) + NormalizeDouble(ld_156 * gd_148,
               ...
               if (li_92 == -1) {
                  ...
                  if (IsTesting()) {
                     ticket_44 = GhostOrderSend(Symbol(), OP_SELL, LotSize, price_164, 25, 0, 0, g_comment_124, Magic, 0, Red);
                     GhostOrderModify(ticket_44, price_164, price_164 + NormalizeDouble(ld_156 * gd_156, Digits), price_164 - NormalizeDouble(ld_156 * gd_148, Digits), 0, Red);
                     continue;
                  }
                  count_8 = 0;
                  WaitUntilContex();
                  while (count_8 < gi_344) {
                     ticket_44 = GhostOrderSend(Symbol(), OP_SELL, LotSize, price_164, 100, 0, 0, g_comment_124, Magic, 0, Red);
                     if (ticket_44 >= 0) break;
                     Sleep(gi_348);
                     RefreshRates();
                     price_164 = normPrice(Bid);
