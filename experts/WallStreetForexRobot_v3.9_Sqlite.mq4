...
int start() {
   ...
   bool     aOk;
   int      aTicket, aType,   aMagicNo;
   datetime aOpenTime;
   double   aLots,   aOpenPrice, aStopLoss,  aTakeProfit,   aProfit;
   string   aSymbol;
   GhostInitSelect(false,SELECT_BY_POS,MODE_TRADES);
   for (int pos_152 = GhostOrdersTotal() - 1; pos_152 >= 0; pos_152--) {
      if (!GhostOrderSelect(pos_152, SELECT_BY_POS, MODE_TRADES)) Print("Error in OrderSelect! Position:", pos_152);
         ...
         aMagicNo    =  GhostOrderMagicNumber();
         aSymbol     =  GhostOrderSymbol();
         
         if (aType <= OP_SELL && aSymbol == Symbol()) {
            if (aMagicNo != Magic) {
               if (aType == OP_BUY) count_136++;
               else count_140++;
            }
            if (aMagicNo == Magic) {
               ld_64 += aProfit;
               if (aType == OP_BUY) ld_48 = (Bid - aOpenPrice) / gd_372;
               else ld_48 = (aOpenPrice - Ask) / gd_372;
               ld_56 += ld_48;
               if (aType == OP_BUY) {
                  count_128++;
                  if (aStopLoss == 0.0 && StealthMode == FALSE) {
                     price_156 = NormalizeDouble(aOpenPrice - StopLoss * gd_372, Digits);
                     price_164 = NormalizeDouble(aOpenPrice + TakeProfit * gd_372, Digits);
                     GhostFreeSelect();
                     GhostOrderModify(aTicket, aOpenPrice, price_156, price_164, 0, Green);
                     break;
                     continue;
                  }
                  if (Bid >= aOpenPrice + TakeProfit * gd_372 || Bid <= aOpenPrice - StopLoss * gd_372 || (CloseLong(aOpenPrice, iwpr_96, iclose_80, iOpen(NULL, PERIOD_M1,
                     1), iClose(NULL, PERIOD_M1, 1)) && TimeCurrent() - aOpenTime > 180) || (FridayExit && DayOfWeek() == 5 && Hour() >= ExitHour && TimeCurrent() - aOpenTime > 180)) {
                     for (int li_148 = 1; li_148 <= MathMax(1, gi_96); li_148++) {
                        RefreshRates();
                        GhostFreeSelect();
                        aOk   =  GhostOrderClose(aTicket, aLots, NormalizeDouble(Bid, Digits), Slippage, Violet);
                        if (aOk) {
                           count_128--;
                        }
                        break;
                  ...
                  if (!(Bid - aOpenPrice > SecureProfitTriger * gd_372 && MathAbs(aOpenPrice + SecureProfit * gd_372 - aStopLoss) >= Point)) 
                     continue;
                  else
                  {
                     GhostFreeSelect();
                     GhostOrderModify(aTicket, aOpenPrice, NormalizeDouble(aOpenPrice + SecureProfit * gd_372, Digits), aTakeProfit, 0, Blue);
                     break;
                  }
                  continue;
               }
               ...
               if (Ask <= aOpenPrice - TakeProfit * gd_372 || Ask >= aOpenPrice + StopLoss * gd_372 || (CloseShort(aOpenPrice, iwpr_96, iclose_80, iOpen(NULL,
                  PERIOD_M1, 1), iClose(NULL, PERIOD_M1, 1)) && TimeCurrent() - aOpenTime > 180) || (FridayExit && DayOfWeek() == 5 && Hour() >= ExitHour && TimeCurrent() - aOpenTime > 180)) {
                  for (li_148 = 1; li_148 <= MathMax(1, gi_96); li_148++) {
                     RefreshRates();
                     GhostFreeSelect();
                     aOk   =  GhostOrderClose(aTicket, aLots, NormalizeDouble(Ask, Digits), Slippage, Violet);
                     if (aOk) {
                        count_132--;
                     }
                     break;
               ...
               if (aOpenPrice - Ask > SecureProfitTriger * gd_372 && MathAbs(aOpenPrice - SecureProfit * gd_372 - aStopLoss) >= Point) 
               {
                  GhostFreeSelect();
                  GhostOrderModify(aTicket, aOpenPrice, NormalizeDouble(aOpenPrice - SecureProfit * gd_372, Digits), aTakeProfit, 0, Red);
                  break;
               }
            }
         }
...
double CalcLots() {
   ...
   if (gd_232 > 0.0 && AutoMM > 0.0) {
      ...
      ld_40 = 0;
      li_48 = 0;
      GhostInitSelect(false,SELECT_BY_POS,MODE_HISTORY);
