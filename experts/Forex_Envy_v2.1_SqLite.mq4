...
void f0_10() {
   ...
   for (int pos_4 = 0; pos_4 < hist_total_0; pos_4++) {
      ...
      if (OrderMagicNumber() == MagicNumber) gd_948 += OrderProfit() + OrderSwap();
   }
}
...
double f0_2(double a_minlot_0) {
   double minlot_8;
   double ld_16 = AccountEquity() - gi_992;
   double ld_24 = gi_996;
   double ld_32 = gi_1000;
   if (gi_996 == 0 || gi_1000 == 0) minlot_8 = a_minlot_0;
   else {
      ld_24 = gi_992 * ld_24 / 100.0;
      Print("tmp=" + ld_16 + ",AccountEquity()=" + AccountEquity() + ",InitEquity=" + gi_992);
      ld_32 /= 100.0;
         ...
         if (OrderMagicNumber() == MagicNumber && OrderSymbol() == Symbol()) {
            ...
            if (OrderType() == OP_BUY) {
               if (OrderStopLoss() == 0.0) {
                  ld_12 = OrderOpenPrice() - gi_984 * Point;
   ...
   for (int pos_4 = OrdersTotal() - 1; pos_4 >= 0; pos_4--) {
      if (OrderSelect(pos_4, SELECT_BY_POS)) {
         if (OrderMagicNumber() == MagicNumber) {
            if (StringFind(OrderComment(), gs__hedged_1156) == -1) {
               f0_16("GetLastLotSize " + ai_0 + ",OrderLots()=" + OrderLots());
               return (OrderLots());
            }
         }
      }
...
int f0_12(bool ai_0 = FALSE) {
   ...
   if (!GlobalVariableCheck("PERMISSION")) {
      ...
      if (!SupportECN) {
         if (ai_0) {
            if (OrderSelect(g_ticket_1148, SELECT_BY_TICKET)) ld_24 = OrderTakeProfit() - MarketInfo(Symbol(), MODE_SPREAD) * Point;
...
int f0_9(string a_symbol_0) {
   ...
   for (int pos_40 = order_total_28 - 1; pos_40 >= 0; pos_40--) {
      if (OrderSelect(pos_40, SELECT_BY_POS, MODE_TRADES)) {
         if (OrderSymbol() == a_symbol_0 && OrderMagicNumber() == MagicNumber) {
            if (OrderType() == OP_BUY) {
               RefreshRates();
...
int start() {
   ...
   if ((100 - BailOutPct) * AccountBalance() / 100.0 >= AccountEquity()) {
      f0_16("AccountBalance=" + AccountBalance() + ",AccountEquity=" + AccountEquity());
      gi_1024 = TRUE;
      li_36 = TRUE;
   }
   if ((gd_1004 + 100.0) * AccountBalance() / 100.0 <= AccountEquity()) gi_1024 = TRUE;
   if (gi_1024) {
      for (int pos_40 = OrdersTotal() - 1; pos_40 >= 0; pos_40--) {
         if (OrderSelect(pos_40, SELECT_BY_POS)) {
            if (OrderMagicNumber() == MagicNumber) {
               f0_16("close #" + OrderTicket());
   ...
   if (SupportECN) {
      ...
      if (OrderSelect(g_ticket_1148, SELECT_BY_TICKET)) order_takeprofit_0 = OrderTakeProfit();
      for (pos_40 = 0; pos_40 < OrdersTotal(); pos_40++) {
         if (OrderSelect(pos_40, SELECT_BY_POS)) {
            if (OrderMagicNumber() == MagicNumber) {
               order_takeprofit_44 = OrderTakeProfit();
               if (order_takeprofit_44 == 0.0 && StringFind(OrderComment(), gs__hedged_1156) == -1) {
                  if (OrderType() == OP_BUY) {
                     if (count_24 > 0) {
               ...
               if (StringFind(OrderComment(), gs__hedged_1156) != -1 && g_cmd_1152 == OrderType()) {
...
double f0_1(int ai_0) {
   double ld_ret_4;
   if (ai_0 == 2) {
      ld_ret_4 = (AccountEquity() / AccountBalance() - 1.0) / (-0.010000000000000000208);
